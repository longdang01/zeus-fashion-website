...
<div ng-controller="TransportController">
    ...
    <div class="grid columns-12 gap-6 mt-5">
        <div class="intro-y g-col-12 d-flex flex-wrap flex-sm-nowrap align-items-center mt-2">
            ...
            data-bs-target="#save-modal-transport"
            ng-click="getTransport(0)"
            >Thêm hình thức vận chuyển</button>
            ...
            <div class="d-none d-md-block mx-auto text-gray-600">Hiển thị <span ng-cloak>@{{ pageSize }}</span> / <span ng-cloak>@{{ transports.length }}</span> hình thức vận chuyển</div>
            ...
            <table class="table table-report mt-n2">
                <thead>
                    <tr>
                        ...
                        <th class="text-wrap">Hình thức vận chuyển</th>
                        <th class="text-center text-nowrap">Mức phí (VNĐ)</th>
                        <th class="text-center text-nowrap">Hành động</th>
                    </tr>
                </thead>
                <tbody>
                    <tr ng-cloak dir-paginate="transport in transports | itemsPerPage:pageSize | filter: { transport_type: keySearch } track by $index" current-page="currentPage" class="intro-x">
                        <td class="text-center w-20">@{{ $index + currentPage * 10 - 9 }}</td>
                        <td class="text-start">@{{ transport.transport_type }}</td>
                        <td class="text-center">@{{ transport.fee | currency:"":0 }}</td>
                        <td class="table-report__action w-40">
                            <div class="d-flex justify-content-center align-items-center">
                                <a class="d-flex align-items-center me-3" href="javascript:;"
                                ng-click="getTransport(transport.id)"
                                >
                                ...
                                href="javascript:;"
                                ng-click="getTransport(transport.id, -1);"
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
    ...
    <!-- BEGIN: Brand  -->
    <div id="save-modal-transport" class="modal fade" tabindex="-1" aria-hidden="true">
                                ...
                                <div class="card card-body" style="background: #f1f5f8;">
                                    <div style="background: #fff;" class="grid columns-12 gap-4 gap-y-3 w-full p-5">
                                        <div class="g-col-12 g-col-sm-6">
                                            <label for="modal-form-1" class="form-label">Hình thức vận chuyển</label>
                                            <input id="modal-form-1" type="text" class="form-control"
                                            placeholder="Giao hàng nhanh, Giao hàng tiết kiệm" 
                                            ng-model="transport.transport_type">
                                        </div>
                                        <div class="g-col-12 g-col-sm-6">
                                            <label for="modal-form-1" class="form-label">Mức phí (VNĐ)</label>
                                            <input id="modal-form-1" type="text" class="form-control"
                                            placeholder="30000, 50000" 
                                            ng-model="transport.fee">
                                        </div>
                                        <div class="g-col-12">
                                            ...
                                            <div class="mt-2">
                                                <div
                                                    ckeditor="options"
                                                    ng-model="transport.description"
                                                ></div>
                                            </div>
                                        </div>
                                    </div>
